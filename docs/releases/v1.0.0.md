...
- [x] Offboarding, Cleanup, Bulk / CSV and Usage reports migrated to tiles
- [x] Security review migrated; CA policies render as facts with the JSON folded
      away (the earlier "still raw JSON" note is obsolete)
- [ ] Connect / Settings / Activity / Run history stay form-and-list pages by
      design — only their headers need the shared treatment
- [x] Playbooks: migrated to tiles — field labels, role profiles for onboarding,
      the report survives switching between the two flows, channels from more
      than one team, live steps + cancel kept
- [x] Offboarding v2: group/team ownership transfer and cancelling organised
      meetings. The zip-archive backup idea is dropped for 1.0 on purpose: the
      server-side copy never routes bytes through the operator's machine, and a
      zip would undo exactly that.
- [x] Activity merged into Run history as a second tab; the separate menu entry
      is gone
- [ ] Security: Conditional Access policies still render as raw JSON
- [ ] Version bumps: `app/app.go` baseVersion, `wails.json` productVersion,
      installer version
- [ ] Wiki push (Permissions.md, Teams-Notifications.md, task-first UI page)
- [ ] VM test: 0.9.0 → 1.0.0 upgrade, clean install, silent in-app update
- [ ] Real-tenant pass over the mirror on users with many teams (channel walk
      cost), and over the filtered audit queries
